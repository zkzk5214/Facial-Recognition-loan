...
| `imgType` | String | 是 | 图片格式，如 `"JPG"`、`"PNG"` |
...
| `dark_score` | Float | 模型给出的暗背景置信度，0~1，保留 4 位小数 |
| `dk_ratio` | Float | 背景区域中暗像素占比（0~1，保留 4 位小数）。0 表示 Stage 1 未通过或无人脸等情况 |
| `br_ratio` | Float | 背景区域中亮像素占比（0~1，保留 4 位小数）。用于区分车内窗光等非纯黑背景场景 |
| `darkness_level` | Float | 图片昏暗程度评分（0~100，保留 4 位小数）。仅当 `is_dark_bg=true` 时有意义，基于背景非暗区的灰度均值取反：越接近 100 越昏黑，0 为正常 |

#### 检测逻辑说明

检测分两阶段：
1. **模型判断**（Stage 1）：用深度学习模型判断整体画面是否为暗场景。若模型认为非暗场景，直接返回 `false`。
2. **CV 验证**（Stage 2）：若模型判定为暗场景，从图片中定位人脸区域，计算人脸以外（且排除脸部下方衣物区域）的背景区域中暗像素比例。需同时满足暗像素占比 > 75% 且亮像素占比 < 10% 才最终判定为暗背景（排除车内等有窗光亮区的场景）。通过后再基于背景非暗区的灰度均值计算 `darkness_level`，给出连续昏暗评分。
...
```
┌─────────┐     ┌─────────────────┐     ┌──────────────────┐
│ 客户端   │     │ /face_register  │     │ /head_detection  │
└────┬────┘     └───────┬─────────┘     └────────┬─────────┘
     │                  │                        │
     │ 1. 用户拍照       │                        │
     │─────────────────>│                        │
     ...
     │<─────────────────│                        │
     │                  │                        │
     │ 3. 携带 faceFeature + 实时帧              │
     │──────────────────────────────────────────>│
     │                  │                        │
     │ 4. 返回 faceSimilarity + msg              │
     │<──────────────────────────────────────────│
     │                  │                        │
     │ 5. 重复步骤 3-4，直到通过或超时              │
     │                  │                        │
...
1. **注册：** 调用 `/face_register`，传入用户正面照片，获取 `faceFeature`。
2. **可选暗背景检测：** 调用 `/dark_bg_check`，检查拍摄环境亮度，如 `is_dark_bg = true` 则提示用户。
