...
Input (np.ndarray BGR image)
    ...
    │     ├─ argmax ≠ 2 or dark_score < 0.95 → return (False, 100, dark_score, 0.0, 0.0, 0.0)
    │     └─ pass → proceed to Stage 2
    │
    └─ Stage 2: _face_detector.get_face_bboxes(img)
          ├─ no faces → return (False, 300, dark_score, 0.0, 0.0, 0.0)
          └─ has faces → detect_bg_darkness(img, face_bbox)
                ├─ bg_pixels == 0 → return (False, -1.0, -1.0) [sentinel: no bg → triggers fallback to stage1]
                ├─ bg_pixels > 0 → return (is_dark, dark_ratio, bright_ratio, darkness_level)

Output: (is_dark_bg: bool, resp_code: int, dark_score: float, dk_ratio: float, br_ratio: float, darkness_level: float)
```
...
| **Return** | `(bool, float, float, float)` — whether background is dark, dark pixel ratio, bright pixel ratio, and darkness level. Returns `(False, -1.0, -1.0, 0.0)` as sentinel when no background pixels exist (face fills image). |
| **Core Logic** | 1. Convert to grayscale (`cv2.COLOR_BGR2GRAY`, BT.601); 2. Expand bbox (up: 10%, other: 0% of bbox size); 3. Clip expanded bbox to image boundaries; 4. Create mask: face = 0, area below face = 0 (exclude clothing), bg = 255; 5. Count dark pixels (gray < `dark_pixel_thresh`) and bright pixels (gray > `bright_pixel_thresh`) in bg region; 6. If `dark_ratio > dark_ratio_thresh` AND `bright_ratio < bright_ratio_thresh` → dark background, then compute `darkness_level` from mean brightness of non-dark bg pixels; 7. Return `(is_dark, dark_ratio, bright_ratio, darkness_level)` |
...
| **Return** | `(is_dark_bg, resp_code, dark_score, dk_ratio, br_ratio, darkness_level)` — final judgment with debug scores |
...
| `dark_bg_check` return tuple format | Caution | `server.py` expects `(bool, int, float, float, float, float)` |
...
- **Dark background image**: Verify `is_dark_bg=True`, `dark_score ≥ 0.95`, `dk_ratio > 0.75`, `br_ratio < 0.1`, `darkness_level > 70`
